...
from utils.verification_metric import VerificationMetrics
...
def create_verification_evaluator(model, metrics, device=None):
    """Factory function for creating a verification evaluator"""
    if device:
        model.to(device)
    ...
    def _inference(engine, batch):
        ...
        with torch.no_grad():
            ...
            img1_f = fliplr(img1)
            img2_f = fliplr(img2)
            feat1_f = model(img1_f)
            feat2_f = model(img2_f)
...
def do_train(
    ...
    evaluator = create_verification_evaluator(model, metrics={'verification': VerificationMetrics()}, device=device)
    ...
    @trainer.on(Events.EPOCH_COMPLETED)
    def log_verification_results(engine):
        if ((engine.state.epoch % eval_period == 0) or (engine.state.epoch == epochs)) and (engine.state.epoch > 1.7*epochs):           
            evaluator.run(val_loader)
            metrics = evaluator.state.metrics['verification']
            logger.info("Verification Results - Epoch: {}".format(engine.state.epoch))
            logger.info("Accuracy: {:.3f}".format(metrics['accuracy']))
            logger.info("EER: {:.3f}".format(metrics['eer']))
            logger.info("AUC: {:.3f}".format(metrics['auc']))
